...
This worksheet is for Windows, it is identical to the [Linux](tutorial_lidar) version but scripts don't have the extension `` and lines are broken by `^` rather than `\`.
